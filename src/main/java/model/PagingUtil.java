...
public class PagingUtil {
	...
	*/
	public static String pagingBootStrapStyle(int totalRecordCount, int pageSize, int blockPage, int nowPage, String page) {
	    return pagingBootStrapStyle(totalRecordCount, pageSize, blockPage, nowPage, page, null, null);
	}
	...
	public static String pagingBootStrapStyle(int totalRecordCount, int pageSize, int blockPage, int nowPage, String page, String searchColumn, String searchWord) {
	    ...
	    // 전체 페이지가 1 이하인 경우 페이지 네비게이션 표시 안 함
	    if (totalPage <= 1) {
	        return "";
	    }
	    ...
	    if (intTemp != 1) {
	        pagingStr += "<li class=\"page-item\">" +
	                "<a class=\"page-link\" href='" + page + NOWPAGE + "=1" +
	                (searchColumn != null && searchWord != null ? "&searchColumn=" + searchColumn + "&searchWord=" + searchWord : "") + "'>" +
	                "<span >&laquo;</span>" +
	                "</a>" +
	                "</li>" +
	                "<li class=\"page-item\">" +
	    ...
	    while (blockCount <= blockPage && intTemp <= totalPage) {  // 페이지 오버를 체크
	        if (intTemp == nowPage) {
	            ...
	            pagingStr += "<li class=\"page-item\"><a class=\"page-link\" href='" + page + NOWPAGE + "=" + intTemp +
	                    (searchColumn != null && searchWord != null ? "&searchColumn=" + searchColumn + "&searchWord=" + searchWord : "") + "'>" + intTemp + "</a></li>";
	        }
