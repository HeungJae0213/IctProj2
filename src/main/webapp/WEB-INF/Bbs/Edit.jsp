	...
	<div class="container">
        <div class="container-fluid">
            ...
            <div class="p-5 gradient-bg text-black">
                <h1>게시글 <small>수정</small></h1>
            </div>
            <c:if test="${not empty errorMsg}">
                   <div class="alert alert-danger alert-dismissible my-2 col-11" >
                   <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                   <strong>수정 실패!</strong>&nbsp;&nbsp;${errorMsg}
                   </div>
            </c:if>
            <form action="<c:url value='/Bbs/edit.ict'/>" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
                <input type="hidden" name="id" value="${record.id}"/>
                <input type="hidden" name="mode" value="edit"/>
                <input type="hidden" name="originalFilename" value="${record.files}"/>
                <div class="mb-3 mt-3">
                    <label for="title" class="form-label">제목</label>
                    <input type="text" class="form-control" id="title" placeholder="제목을 입력하세요" name="title" value="${record.title}">
                </div>
                <div class="mb-3">
                    <label for="content" class="form-label">내용</label>
                    <textarea placeholder="내용을 입력하세요" class="form-control" rows="5" id="content" name="content">${record.content}</textarea>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <c:if test="${not empty record.thumbnail}">
                        <p>현재 썸네일: <img src="<c:url value='/uploadthumb/${record.thumbnail}'/>" alt="썸네일" style="max-width: 100px;"></p>
                    </c:if>
                </div>
